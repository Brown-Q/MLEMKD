...
class RecurrentRGCN(nn.Module):
    ...
    def soft_label(self, test_graph, num_rels, static_graph, all_triples, use_cuda):
        with torch.no_grad():
            evolve_embs, _, r_emb, _, _ = self.forward(test_graph, static_graph, use_cuda)
            embedding = F.normalize(evolve_embs[-1]) if self.layer_norm else evolve_embs[-1]
    ...
    def get_distillation_loss(self, glist, all_triples, static_graph, entity_pro, relation_pro, quality_entity_score,
        ...
        if self.entity_prediction:
            scores_ob = self.decoder_ob.forward(pre_emb, r_emb, all_triples).view(-1, self.num_ents)
            loss_ent_quality = torch.nn.MSELoss()(torch.mul(quality_entity_score, torch.gather(scores_ob, 1, all_triples[:, 2].unsqueeze(-1))), torch.mul(quality_entity_score, torch.gather(entity_pro, 1, all_triples[:, 2].unsqueeze(-1)))) * 0.1
            loss_ent_value = self.loss_e(torch.mul(value_entity_score, scores_ob), torch.mul(value_entity_score, entity_pro)) * 0.00004
            # print('------------------')
            # print(self.loss_e(scores_ob, all_triples[:, 2]))
            # print(loss_ent_quality)
            # print(loss_ent_value)
            loss_ent += self.loss_e(scores_ob, all_triples[:, 2]) + loss_ent_quality + loss_ent_value
    ...
    def get_distillation_loss_reverse(self, glist, all_triples, static_graph, entity_pro, relation_pro,
